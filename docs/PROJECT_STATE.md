...
- Gate 7 semantic Remotion animation: initial plan-driven 24-second slice with approved local voice and music masters, deterministic two-pose timing-driven lip sync, and hash-bound mix decisions is implemented and proven; phoneme/viseme sync, cue-placed custom SFX, full episode coverage, and visual polish remain pending.
...
StoryStage is not yet the complete script-to-finished-episode product. It does not yet automatically create final ChatGPT art, record voice acting inside the app, place custom SFX cues, acquire licensed stock/archive media, or route Blender scenes. It can independently import, audition, approve, bind, and render external local WAV voice and music masters; compile deterministic two-pose mouth timing; and save explicit music/transition-SFX mix decisions. The manual subscription-backed image workflow is intentional: no API key, browser cookie, password, or ChatGPT session is stored by the app.
