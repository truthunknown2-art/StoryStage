...
`CV-001-B` is implemented for Pro audit. A pure compiler now turns three strict beat intents—notice prop, reach and pick up, react and present—into three independently hashed, shot-bound motion programs. Identical canonical input produces identical scene, binding, and program hashes; editing beat two leaves beats one and three byte-identical. The production composition accepts a unique binding array, rejects stale, duplicate, missing, mismatched, or truncated bindings, derives one shared ground-to-hand prop transform across the scene, and renders all three performances through the same frame-evaluated rig used by final H.264 output. The ten-second local proof checks thirteen frames twice with matching pixel hashes and records zero position, rotation, and scale discontinuity at pickup. It remains engineering art, not the approved Kids Show Pack or creator-facing workflow.
