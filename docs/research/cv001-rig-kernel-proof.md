...
Status: engineering motion proof, not final character art or a finished creator workflow.
...
- An optional `directedBeatProgram` path inside `ProductionComposition`, so the same frame-evaluated component can replace the legacy pose-swap performance for a directed shot.
- A repeatable render command: `pnpm --filter @storystage/render-worker render:cv001-rig-proof`.

The local proof render produces an H.264 MP4, six exact-frame PNGs, and a JSON report under ignored `artifacts/CV-001/rig-kernel-proof/`. The committed [contact sheet](../design/creator-first-reset/cv001-rig-kernel-proof-contact-sheet.png) shows frames 0, 24, 48, 70, 100, and 119.
