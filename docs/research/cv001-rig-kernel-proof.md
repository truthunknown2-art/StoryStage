...
- CV-001-B supersedes the former loose shot-binding entry point with a complete compiled-scene envelope inside `ProductionComposition`. Its self-hashed plan identity and all three exact bindings must validate before any directed shot renders.
- A repeatable render command: `pnpm --filter @storystage/render-worker render:cv001-rig-proof`.

The evidence accepted at `0654ce2` rendered through `StoryStageProduction` and proved plan, program, and rendered duration were all 120 frames. The retained `render:cv001-rig-proof` command now isolates the same 120-frame kernel in its registered diagnostic composition, while the stricter CV-001-B proof owns current multi-shot production integration. It produces an H.264 MP4, seven exact-frame PNGs, and a JSON report under ignored `artifacts/CV-001/rig-kernel-proof/`. The report records the lantern's full free and attached world transform at frame 48 and requires position, rotation, and scale deltas below 0.001; all three current deltas are zero. The committed [contact sheet](../design/creator-first-reset/cv001-rig-kernel-proof-contact-sheet.png) includes adjacent frames 47 and 48 so the pickup boundary is directly inspectable.
