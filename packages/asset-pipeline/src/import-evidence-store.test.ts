...
const roots: string[] = [];
const checkpoints: ImportEvidenceCheckpoint[] = ["candidate-bundle-written", "import-record-written", "validation-report-written", "evidence-committed"];
const expectedContentHash = "a".repeat(64);
const files = {candidateBundle: '{"schemaVersion":"1.0"}\n', importRecord: `${JSON.stringify({contentHash: expectedContentHash})}\n`, validationReport: '{"schemaVersion":"1.0"}\n'};
const readCommittedContentHash = async (evidenceRoot: string) => (JSON.parse(await readFile(join(evidenceRoot, "import-record.json"), "utf8")) as {contentHash: string}).contentHash;
...
describe("atomic import evidence directory", () => {
  for (const crashPoint of checkpoints) {
    it(`recovers idempotently after ${crashPoint}`, async () => {
      ...
      await expect(commitImportEvidenceDirectory({stagingRoot, expectedContentHash, files, readCommittedContentHash, transactionId: `crash-${crashPoint}`, onCheckpoint: (checkpoint) => { if (checkpoint === crashPoint) throw new Error(`simulated crash after ${checkpoint}`); }})).rejects.toThrow("simulated crash");
