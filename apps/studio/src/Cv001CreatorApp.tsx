...
import {ArrowRight, Check, Clapperboard, Feather, FileText, Play, Sparkles, Trees} from "lucide-react";
...
type CreatorScreen = "creator-create" | "creator-studio" | "draft-review";
...
const countParagraphs = (value: string) => value.trim() ? value.trim().split(/\r?\n[\t ]*\r?\n/).filter((paragraph) => paragraph.trim()).length : 0;
...
export function Cv001CreatorApp({onOpenLegacy}: {onOpenLegacy: () => void}) {
  ...
  const restoredDraft = useMemo(() => {
    ...
    try {
      ...
      return {project: null, notice: "The saved script breakdown could not be verified, so StoryStage removed it."};
    }
  }, []);
  const [draftProject, setDraftProjectState] = useState<Cv002Project | null>(restoredDraft.project);
  const [studioEntryMode, setStudioEntryMode] = useState<StudioEntryMode>("new-first-cut");
  const [notice, setNotice] = useState<string | null>(restored.notice ?? restoredDraft.notice);
  const [replaceConfirmOpen, setReplaceConfirmOpen] = useState(false);
  const isLanternRoute = grammar === "kids-adventure" && script.trim() === CV001_DEFAULT_SCRIPT.trim();
  ...
  const draftScriptError = useMemo(() => {
    if (scriptWords < 80 || scriptWords > 400) return `Paste 80 to 400 words for a direction draft. This script has ${scriptWords}.`;
    if (scriptParagraphs < 1 || scriptParagraphs > 6) return `Use one to six non-empty paragraphs. This script has ${scriptParagraphs}.`;
    return null;
  ...
  const createDraft = () => {
    try {
      ...
      setNotice(caught instanceof Error ? caught.message : "StoryStage could not create this draft breakdown.");
    }
  };

  const submitProject = () => isLanternRoute ? createFirstCut() : createDraft();
  ...
  if (screen === "draft-review" && draftProject)
    return <Cv002DraftReview onBack={() => setScreen("creator-create")} onProjectChange={saveDraftProject} project={draftProject} />;
  ...
  return (
    <main className="cv-create">
      <header className="cv-create-topbar">
        ...
        <span className="cv-prototype-pill"><Sparkles size={13} />Script-to-direction beta</span>
      </header>

      <div className="cv-create-layout">
        <section className="cv-create-copy">
          <p className="cv-kicker">New production</p>
          <h1>Start with the words.<br />Find the story.</h1>
          <p>Paste a script, choose how it should tell its story, then review the scenes, beats, shots, performance, sound, and editorial rhythm before production begins.</p>
          <div className="cv-create-proof">
            <span><Check size={14} /></span>
            <div><strong>Animation starts with readable beats</strong><small>The lantern demo proves articulated motion. New scripts first get an honest, editable direction plan.</small></div>
          </div>
        </section>

        <form className="cv-create-form" onSubmit={(event) => {event.preventDefault(); submitProject();}}>
          <header><p>Project setup</p><h2>What are we making?</h2></header>
          ...
          <div className={scriptError ? "cv-script-help is-error" : "cv-script-help"} id="cv-script-help">
            <span>{scriptError ?? (isLanternRoute ? "Animated lantern demo · 3 beats ready" : `${scriptWords} words · ${scriptParagraphs} paragraphs · ready for breakdown`)}</span>
            <small>{isLanternRoute ? "This exact scene has an assigned rig and motion template." : "New scripts stop at an editable direction draft until animation templates are assigned."}</small>
          </div>
          {grammar === "weird-history" && script === CV001_DEFAULT_SCRIPT ? <button className="cv-load-sample" onClick={() => {setTitle("The Alaska Bargain"); setScript(HISTORY_SAMPLE);}} type="button"><FileText size={15} />Load a Weird History sample</button> : null}

          <fieldset>
            <legend>Project grammar</legend>
            <div className="cv-choice-grid is-two">
              <button aria-pressed={grammar === "kids-adventure"} className={grammar === "kids-adventure" ? "is-selected" : ""} onClick={() => setGrammar("kids-adventure")} type="button">
                <span><Trees size={18} /></span><strong>Kids Adventure</strong><small>Character-led · warm holds</small>{grammar === "kids-adventure" ? <Check size={14} /> : null}
              </button>
              <button aria-pressed={grammar === "weird-history"} className={grammar === "weird-history" ? "is-selected" : ""} onClick={() => setGrammar("weird-history")} type="button">
                <span><Feather size={18} /></span><strong>Weird History</strong><small>Evidence-led · fast cuts</small>{grammar === "weird-history" ? <Check size={14} /> : null}
              </button>
            </div>
          </fieldset>
          ...
          <fieldset>
            ...
            <div className="cv-choice-grid is-three">
              <button aria-pressed={grammar === "kids-adventure"} className={grammar === "kids-adventure" ? "is-selected" : ""} disabled={grammar !== "kids-adventure"} type="button"><span className="cv-style-swatch is-cut-paper" /><strong>Cut-paper forest</strong><small>{isLanternRoute ? "Animated demo" : "Direction reference"}</small>{grammar === "kids-adventure" ? <Check size={14} /> : null}</button>
              ...
              <button aria-pressed={grammar === "weird-history"} className={grammar === "weird-history" ? "is-selected" : ""} disabled={grammar !== "weird-history"} type="button"><span className="cv-style-swatch is-collage" /><strong>Editorial collage</strong><small>Direction reference</small>{grammar === "weird-history" ? <Check size={14} /> : null}</button>
            </div>
          </fieldset>

          <button className="cv-create-action" disabled={Boolean(scriptError) || !title.trim()} type="submit">
            {isLanternRoute ? <><Play fill="currentColor" size={16} />Create animated first cut</> : <><ArrowRight size={17} />Break script into scenes</>}
          </button>
          <p className="cv-create-boundary">{isLanternRoute ? "Prototype art · real articulated motion · no voice or export yet" : "Draft breakdown only · no generated art, voice, animation, or export"}</p>
      ...
      {draftProject ? <button className={`cv-continue-card cv-continue-draft ${project ? "has-animation-card" : ""}`} onClick={() => setScreen("draft-review")} type="button">
        <span>Continue direction draft</span><strong>{draftProject.title}</strong><small>{draftProject.graph.scenes.length} scenes · {draftProject.graph.scenes.flatMap((scene) => scene.beats).length} beats · {draftProject.grammar === "kids-adventure" ? "Kids Adventure" : "Weird History"}</small>
      </button> : null}
