...
Commit `dad474d` — Inbox v6 eligibility-cardinality correction:

- **Root cause.** The v5 UI predicate counted reaction _events_ (`shots.some(...)`) while the interpreter counts _(event, shot) pairs_ (`flatMap`): one reaction event linked by two eligible shots read as one eligible candidate in the UI but as ambiguous to the interpreter — and the inline comment falsely claimed the predicates mirrored.
- **Fix.** `listDirectorReactionDelayCandidates` in `packages/story-engine/src/director/director-patch.ts` is now the single eligibility source, consumed by both `proposeDirectorPatch` (identical behavior) and the Studio shell gating via `@storystage/story-engine/director-alpha`. The two paths cannot drift by construction. Director Alpha import-boundary audit stays green. (Story-engine touch was explicitly assigned by the v6 inbox instruction.)
- **Regression fixture.** New engine test forges a schema-valid project (full reseal chain) with one reaction event linked by two eligible shots: the shared list reports two candidates and the interpreter throws `/more than one possible reaction event/i`. The control therefore stays unavailable for exactly the cases the interpreter rejects. Existing no-reaction (0 candidates) and command-clearing regressions kept; studio suite 81/81.
...
| **`pnpm verify` (repository root, at `dad474d`)**                   | **exit 0** — 19 capability assets verified; privacy 580 files passed; `eslint .` 0 errors (2 pre-existing render-worker flicker warnings in `kvp001-proof.ts`, outside slice scope); typecheck clean across 10 projects; all tests green: story-engine 254/254 (incl. `director-alpha-boundary` and the ambiguity-cardinality regression), studio 81/81, desktop 11/11, contracts/fixtures/orchestration/asset-pipeline/remotion-runtime/asset-worker/render-worker all passed. (An earlier run at `e13292a` flaked once on `apps/desktop delivery-store.test.ts` — 5000 ms timeout under parallel load, unrelated to this branch, green on rerun; test left untouched per Codex guidance and flagged for the flake lane.) |
