...
| Beat cards (image-led)         | real           | `DirectorFrameThumbnail` renders a deterministic proxy approximation of the beat's frame from the sealed episode plan; click selects the beat (`selectDirectorWorkspaceBeat`) and seeks the Player to the beat's exact resolved start frame |
...
| "Direct this beat" input + Preview change (`DirectorCommandPanel`)                                    | real, **gated**             | Mounted only when the selected beat has exactly one concrete reaction event (the same acceptance predicate as `proposeDirectorPatch`); then `proposeDirectorPatch` runs against the current director project, errors shown verbatim, success opens `DirectorChangePreview` |
| "Structured direction unavailable" note                                                               | real (honest fallback)      | Shown in place of the command input on beats without a concrete reaction event; states the limitation and points at the Visual/Motion controls                                                                                                                             |
