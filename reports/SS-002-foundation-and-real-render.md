...
- `pnpm verify`: privacy check, lint, all package typechecks, and 105 tests across fixtures, contracts, story-engine, desktop, asset-pipeline, studio, asset-worker, and render-worker
...
- Completed 24-second approved MP4s can now play and seek inside the editor, transition overrides visibly reach deterministic Remotion frames, and treatment changes rebuild the visual acquisition route; unrendered live Remotion preview and full-length approved playback are not complete.
