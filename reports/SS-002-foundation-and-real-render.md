...
- F - real path: generation job -> manifest-bound staging -> exact three-file evidence -> Sharp normalization -> contact sheet -> selected set -> manifest -> validation -> moving diagnostic -> selected review -> immutable promotion -> approved review -> approval transaction -> new revision -> approved hash-bound WAV master -> reviewed mix -> profile-specific mouth cues -> exact recompile -> two approved Remotion renders with AAC -> decoded-frame comparison.
...
- approved production bundle hash `f82233ec377e1b58a14096b90c2d46c69b5ed39094a9d66ead571e9c112f1c63`
...
- diagnostic report hash `13005de8c7c91448f5d0513e0ca75cd1796164f03539b27d81161361250abf1c`
...
- `pnpm verify`: privacy check, lint, all package typechecks, and 98 tests across fixtures, contracts, story-engine, desktop, asset-pipeline, studio, asset-worker, and render-worker
...
- Spoken text, frame retiming, explicit editor timing locks, local WAV import/audition/approval/render binding, deterministic two-pose mouth timing, and reviewed dry-music/transition-SFX mix decisions are implemented. A real user voice master has not yet been supplied; the proof uses a deterministic engineering tone. Phoneme/viseme sync, imported music masters, licensed archive/stock ingestion, and automatic Blender routing are not implemented.
