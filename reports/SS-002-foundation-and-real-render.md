...
- F - real path: generation job -> manifest-bound staging -> exact three-file evidence -> Sharp normalization -> contact sheet -> selected set -> manifest -> validation -> moving diagnostic -> selected review -> immutable promotion -> approved review -> approval transaction -> new revision -> approved hash-bound voice and music WAV masters -> reviewed mix -> profile-specific mouth cues -> exact recompile -> two approved Remotion renders with AAC -> decoded-frame comparison.
...
- approved production bundle hash `aafacdf27c0b50fc9e305dbef47691817df1690bc478ae0d8f75ce6c2136fc8e`
...
- diagnostic report hash `f5275d1caaceac4e7d3e3ab355a7b15a301147e9059d9d04dfa1f24d91cf872d`
...
- `pnpm verify`: privacy check, lint, all package typechecks, and 101 tests across fixtures, contracts, story-engine, desktop, asset-pipeline, studio, asset-worker, and render-worker
...
- Spoken text, frame retiming, explicit editor timing locks, separate voice/music WAV import-audition-approval-render binding, deterministic two-pose mouth timing, and reviewed music/transition-SFX mix decisions are implemented. Real user voice and music masters have not yet been supplied; the proof uses two distinct deterministic engineering tones. Phoneme/viseme sync, custom SFX cue placement, licensed archive/stock ingestion, and automatic Blender routing are not implemented.
