...
| Bounded MCP                     | Exactly one server, one resource, two exact schemas; tool arguments validated at start/completion; proposal arguments must equal receipt     | `proposal-roundtrip.test.ts` — schema drift and argument/receipt correlation                      | Durable project context is B3                                                           |
...
| No project mutation             | Failure-gate generation runs against a temporary decoy project and verifies identical before/after SHA-256                                   | `no-project-mutation.json` and cited test                                                         | This is not a production-project integration test                                       |
